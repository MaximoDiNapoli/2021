...
var connection = mysql.createConnection({
	...
	port: "3306",
	user: "root",
	password: "Kalos kai agathos",
	database: "eCommerce"
});
let current: Date = new Date();
let cDate: String = current.getFullYear() + '-' + (current.getMonth() + 1) + '-' + current.getDate();
let cTime: String = current.getHours() + ":" + current.getMinutes() + ":" + current.getSeconds();
let dateTime: String = cDate + ' ' + cTime;
...
app.get( "/productos", ( req, res ) => {
	...
	if(req.query.busqueda != undefined){
		...
		usado = req.query.usado.toString()
	}
	else{
		usado = "%%"
	}
...
app.post( "/usuarios/:idUsuario/fav/:producto", ( req, res ) => {
	res.set({ 'Content-Type': 'application/json; charset=utf-8' });
	var idUsuario: number = parseInt(req.params.idUsuario);
	var idProducto: number = parseInt(req.params.producto);
	console.log("Usuarios");
	console.log(idUsuario);
...
app.post( "/usuarios/:idUsuario/compras/:idProducto/:cantidad", ( req, res ) => {
	...
	var cantidad: number = parseInt(req.params.cantidad);
	console.log("Usuarios");
	console.log(idUsuario);
	console.log(idProducto);
...
app.post( "/usuarios/:idUsuario/calificaciones/:idCompra/:calificacion", ( req, res ) => {
	var idUsuario: number = parseInt(req.params.idUsuario);
	var idCompra: number = parseInt(req.params.idCompra);
	var calificacion: number = parseInt(req.params.calificacion);
	console.log("Usuarios");
	console.log(idUsuario);
	console.log(idCompra);
	console.log(calificacion);	
	var id_comprador: number;
	var id_vendedor: number;
	var id_producto: number;
